...
- [x] **Bug real de mail de leads + interruptor manual (2026-07-27)**: diagnosticado en el VPS (con prueba controlada, ver detalle abajo en "Notas importantes") que el mail de leads nunca salía por un bug de orden de imports ESM — arreglado con transporter lazy en `email.util.js`. Además, a pedido del usuario, se agregó un interruptor manual para no depender de que el mail funcione bien desde el primer minuto: nuevo `Settings.model.js` (documento único `_id: 'global'`) + `utils/settings.util.js` (`isLeadEmailsEnabled`/`setLeadEmailsEnabled`), chequeado al principio de `sendLeadEmail()` — **arranca en `false` por default** (si no existe el documento, no manda mail). Endpoints `GET`/`PATCH /api/leads/email-setting`. Switch visible en el toolbar de la sección Leads (`LeadEmailToggle` en `Leads.js`) — el lead SIEMPRE se guarda y aparece en la sección pase lo que pase con este switch; el switch solo frena el mail de notificación. Probado end-to-end: toggle prende/apaga y persiste, y un lead creado con el switch en `false` se guarda igual (201) sin intentar mandar mail.
- [x] **IDs de propiedades cargadas a mano (2026-07-27)**: `createProperty` generaba `id: Date.now()` (13 dígitos, ej. `1785180328108`) muy distinto del rango de Tokko (7-8 dígitos, ej. `8169882`) y con riesgo teórico de colisión en dobles clicks rápidos (misma milisegundo → `E11000 duplicate key` en el segundo). Nuevo `models/Counter.model.js` con contador atómico (`findOneAndUpdate` + `$inc`, sin condición de carrera) que arranca en `900000000` — 9 dígitos, claramente separado del rango real de Tokko para siempre. `createProperty` ahora usa `await nextManualPropertyId()`. Probado en local: crear propiedad → subir foto (`POST /:id/photos`) funciona de punta a punta (201 en ambos pasos) tanto antes como después del cambio — el 404 reportado en la carga de fotos no se pudo reproducir contra el código actual, puede haber sido una propiedad de prueba ya borrada o un choque de IDs de la versión vieja; si vuelve a aparecer, revisar con el Network tab del navegador qué id exacto devuelve el 404.
...
- [ ] **Integración MercadoLibre (en curso desde 2026-07-21)** — doc oficial: https://developers.mercadolibre.com.ar/es_ar/introduccion-guia-de-inmuebles
  ...
  **OAuth completado en el VPS (2026-07-27)**: probada la sincronización de 1 sola propiedad con éxito. Falta decidir cuándo correr el sync masivo sobre el resto (~210 propiedades).
  ...
  **Pendiente:**
  - Decidir cuándo correr el sync masivo sobre el resto de las propiedades (ya probado con 1)
...
- [ ] Integración ZonaProp (stub en `zonaprop.controller.js`, sin acceso todavía — el usuario está evaluando armar un bot que replique cambios del CRM ahí ante la falta de API oficial)
- [x] **Reportes reales de MercadoLibre (2026-07-27)**: reemplazado el placeholder por un reporte de portfolio completo (todo lo que da la API de ML, no por propiedad — eso ya existe en la tab Estadísticas de cada ficha). Nuevo `getPortfolioMetricsReport()` en `mercadolibreMetrics.service.js` (agrega todos los `MlMetricSnapshot` del rango elegido, sin filtrar por propiedad) + endpoint `GET /api/mercadolibre/reports?days=7|30|90`. Frontend: `Reportes.js` tiene ahora selector de rango, tiles clickeables (visitas/preguntas/teléfono/whatsapp/leads, mismo patrón que `MlStats.js` de la ficha) con `LineChart` de la serie diaria del metric activo, barras de leads por tipo de contacto (whatsapp/pregunta/llamado/visita/cotización) y dos tablas top-10 (propiedades más visitadas / con más leads, con link a la ficha). Probado end-to-end con snapshots de prueba insertados a mano y borrados después — la agregación, el ranking y el join con `Property` funcionan.
...
- [x] **Gestor de usuarios (2026-07-27)**: el nav item "Usuarios" del sidebar ya está gateado por `superAdminOnly: true` (`session?.role === 'SUPERADMIN'`, ver `Sidebar.js`). El usuario confirmó que va a ser el único SUPERADMIN — verificado en la base local, hoy solo existe una cuenta con ese rol (`barriosarom@gmail.com`). No hace falta un flag `isOwner` aparte; si en el futuro se suma otro SUPERADMIN que no deba ver esta sección, ahí sí hay que revisar el mecanismo de gating.
...
- **Cuidado con crear `transporter`/clientes de librerías a nivel de módulo**: `email.util.js` tenía `nodemailer.createTransport(...)` corriendo al importar el archivo, con `auth: { user: process.env.SMTP_USER, ... }`. En `index.js`, el `import router from './routes/index.router.js'` (que arrastra `email.util.js` transitivamente) se resuelve *antes* de que `dotenv.config()` llegue a ejecutarse (los imports ESM siempre se resuelven antes que el cuerpo del módulo, sin importar el orden textual) — el transporter quedaba armado con credenciales `undefined` para siempre, mientras el chequeo `if (!SMTP_USER || !SMTP_PASS) return` sí veía las env vars ya cargadas (corre en cada llamada, no al importar) y dejaba pasar el envío igual. Resultado: `EAUTH: Missing credentials` silencioso (solo a consola) en todos los mails de leads, sin que nada en el `.env`/contraseña estuviera mal. Fix: el transporter ahora se crea recién en el primer uso (`getTransporter()`), no al importar. Si se agrega otro cliente de librería en el futuro (SDK de un portal, otro servicio de mail, etc.), crearlo lazy o mover `dotenv.config()` a la primera línea de `index.js`.
