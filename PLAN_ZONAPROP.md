...
8. ✅ **Credenciales de producción recibidas (2026-09-03/07)**: mismo `client_id` (`braices_fernandez`), `client_secret` nuevo, mismo `codigoInmobiliaria` (`30023986`). Confirmado en vivo contra `https://api-zp-open.navent.com`: login OK, `/v1/inmobiliarias` confirma la cuenta real, créditos idénticos a los que se veían en sandbox (`DESTACADO:7`, `HOME:0`, `SIMPLE:1` — confirma que sandbox reflejaba datos reales de lectura, pero aislado en escritura, ver punto de abajo).
...
## 12. Producción real: limitación de plan "API Free" y fallback de leads por polling (2026-09-03/07)

- **Verificado que sandbox nunca tocó datos reales**: antes de escribir nada más, se comparó el aviso `PRO8565024` en sandbox vs producción. Producción muestra `fechaModificacion: 2026-07-28` (anterior a toda esta sesión) y ninguno de los warnings específicos que generó mi código de prueba (`WARN-0101`, `WARN-0213`) — confirma que sandbox es de escritura aislada, aunque comparta lectura de catálogo/créditos/avisos con producción (mismos `idAvisoNavplat`, `codigoAviso`, `claveInterna` en ambos ambientes).
- **Limitación real de la cuenta (avisada por Navent, confirmada en vivo)**: la integración está en plan **"API Free"**, que NO incluye:
  - `GET/PUT /v1/configuracion/callbacks*` → probado en producción: `500 {"code":"error_inesperado","message":"Access is denied"}`.
  - `GET .../avisos/{codigoAviso}/calidad` (no probado directo, pero mencionado explícitamente por Navent junto con callbacks).
  - Se necesita pedir **"Rol Premium"** a Navent para desbloquear esto — no es un bug, es una limitación de plan contratado. `configureZonapropCallbacks` ahora detecta este error puntual (`"Access is denied"`) y devuelve un 422 con mensaje explicativo en vez de un 502 crudo.
- **Fallback encontrado y ya implementado — polling de leads**: `GET /v2/inmobiliarias/{cod}/mensajes?fromDate=yyyyMMdd` **sí funciona en plan Free**, probado en vivo contra producción: trajo **161 contactos reales de los últimos 90 días** que el CRM nunca había capturado (la integración no existía antes). No funciona igual en sandbox (`409 "La inmobiliaria no pertenece a la integración 20352"` — particularidad del ambiente de prueba, no afecta producción).
  - `zonaprop.service.js`: `getMensajesPage()` / `getAllMensajes(fromDate)`.
  - `zonaprop.controller.js`: `pollZonapropLeads({ sinceDays, notify })` — crea `Lead` (con `externalId: "zp-<id>"` para no duplicar en re-consultas) + dispara `sendLeadEmail` **solo si `notify: true`**. `pollZonapropLeadsHandler` (fire-and-forget, mismo criterio que sync-all/reconcile) expone `POST /api/zonaprop/leads/poll?sinceDays=N&notify=true|false`, con `notify` por defecto `true` solo si `sinceDays<=3`.
  - **Importante, corregido tras aviso del usuario**: un backfill histórico (`sinceDays=90`) NO manda mail — mandar 161 mails de "nuevo lead" por contactos de hace meses sería spam. Solo el polling incremental corto (leads realmente nuevos) notifica.
  - `Lead.model.js`: campo nuevo `externalId` (index, sparse) para el dedupe.
  - Cron nuevo `zonapropLeads.cron.js`: corre cada 15 minutos, `sinceDays: 2` (margen de 2 días por si el server estuvo caído), `notify: true` — este es el reemplazo funcional de los callbacks para el caso de uso real (leads), ya registrado en `index.js`.
  - UI: botón "Importar leads históricos" en la card de ZonaProp (Difusión) — dispara el backfill de una vez (90 días, sin mail), pensado para correr una sola vez. El polling incremental normal no necesita botón, corre solo.
- **Link "Ver aviso" (2026-09-07)**: la API de Navent nunca devuelve la URL pública del aviso (relevado a fondo: ni en el aviso completo, ni en `/status`, ni en `/avisos/online/resumen` — solo URLs de imágenes). Se encontró y confirmó en vivo contra la web real de ZonaProp (dos avisos distintos, con `curl` + User-Agent de navegador) que `https://www.zonaprop.com.ar/propiedades/clasificado/propiedad-{idAvisoNavplat}.html` resuelve a la página real sin importar el slug — ZonaProp solo valida el ID numérico final. `buildAvisoUrl()` en `zonaprop.service.js`, usado en `syncProperty` y `reconcileExistingListings` para poblar `difusion.zonaprop.url`. Agregado el link "Ver aviso" en ambas UI (card general de Difusión y card dentro de la ficha de propiedad), mismo patrón que MercadoLibre.
- **Selector de plan (2026-09-07)**: faltaba el equivalente al selector de nivel de MercadoLibre. Agregado `updatePlan(propertyId, tipoDePublicacion)` en el service (llama a `syncProperty` con `forcePlan`, reusa todo el manejo de errores existente — si no hay créditos del plan elegido, `ERR-0502` queda en `last_error` igual que cualquier otro error), endpoint `PATCH /api/zonaprop/plan/:propertyId`, y el `<select>` en la card de la ficha de propiedad (mismo patrón/clases CSS que el de MercadoLibre, `ml-tier-row`/`ml-tier-select`).
- **Bug encontrado por el chequeo de salud en el VPS (2026-09-07)**: `reconcileExistingListings()` saltaba de largo (`alreadyLinked++`) las propiedades que ya tenían `codigoAviso` de antes de hoy, así que nunca les completaba el `url` agregado recién — quedaban con el campo vacío aunque ya tuvieran `idAvisoNavplat` guardado. Corregido: ahora, al saltar una propiedad ya vinculada, si le falta `url` la completa igual, calculada 100% local a partir de `idAvisoNavplat` que ya está en Mongo — **cero llamadas a la API de ZonaProp**, no hay riesgo. Correr "Reconciliar existentes" de nuevo soluciona el gap para las propiedades existentes.
...
## 13. Chequeo de salud en producción (2026-09-07, vía Claude Code corriendo en el VPS)

De solo lectura, sobre 204 propiedades elegibles: 160 publicadas, 28 sin `codigoAviso` (nunca publicadas), 16 con `last_error`. Hallazgos:

- **12 de los 16 errores son obsoletos**: quedaron guardados de cuando el `.env` todavía apuntaba a sandbox (fechados 2026-09-05, `zone: api-zp-sandbox-open.navent.com`, error de Cloudflare 521) — no reflejan el estado real actual. Se limpian solos corriendo un sync/reconcile fresco sobre esas propiedades (queda pisado por el resultado nuevo).
- **4 son reales y vigentes: `ERR-0502` (sin créditos)**, consistente con que `SIMPLE` bajó a **0 disponibles** (antes había 1 — se ve que se consumió publicando alguna propiedad nueva). Esto es una restricción de negocio real, no un bug: ninguna propiedad que necesite plan `SIMPLE` se puede publicar/actualizar por primera vez hasta que se liberen o compren más créditos. Las que ya estaban publicadas en `SIMPLE` siguen bien (actualizarlas no consume un crédito nuevo).
- **1 error raro: `codigoAviso`/`Property.id` = `900000006`, "Invalid URL"**. Ese ID no tiene forma de ID de Tokko (8-9 dígitos típicos de Tokko empiezan distinto) — probablemente una propiedad cargada a mano en el CRM con datos incompletos (geo_lat/geo_long o alguna URL de foto rota) que rompe la construcción de una URL interna. Afecta a 1 sola propiedad, no es urgente — pendiente confirmar con el usuario si esa propiedad puntual necesita estar en ZonaProp o es un registro de prueba.
- **`difusion.zonaprop.url` vacío en las muestreadas**: esperable, `buildAvisoUrl()` es de hoy mismo y esas propiedades no se habían vuelto a sincronizar/reconciliar todavía. **Corregido** (ver más abajo): `reconcileExistingListings()` ahora completa `url` también para las que ya estaban vinculadas, sin llamar a la API.

## Cómo retomar

**Pendiente inmediato (2026-09-07):**
1. Correr **"Reconciliar existentes"** de nuevo — con el fix de arriba, esta corrida completa el `url` faltante en las propiedades ya vinculadas (sin tocar la API de ZonaProp).
2. Confirmar con el usuario si la propiedad `900000006` es real o de prueba, antes de invertir tiempo en depurar el "Invalid URL".
3. Créditos `SIMPLE` en 0: avisarle al usuario que no se pueden publicar propiedades nuevas en ese plan hasta que se liberen/compren créditos — no requiere acción de código.
4. **NO** apretar "Configurar callbacks" en producción — va a fallar (plan Free no lo soporta), ya está manejado con un mensaje claro en vez de un error crudo. Si en algún momento quieren tiempo real + calidad de aviso, hay que pedirle a Navent el upgrade a "Rol Premium".

**Deploy 2026-09-02 (ya en producción):** commits `db598e0`, merge `2e4f241`, `bf810d8` (botón callbacks), `e9398eb` (fix timeout reconcile), `37c0ed8` (polling de leads), `d4c5e27` (link "Ver aviso"). **Pendiente de pushear** (2026-09-07): selector de plan + fix del backfill de `url` en reconcile.
