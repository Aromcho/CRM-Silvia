...
1. Actualizar el `.env` de producción con las credenciales reales: `ZP_API_BASE=https://api-zp-open.navent.com`, `ZP_CLIENT_SECRET` nuevo (el de sandbox ya no sirve para producción), mismo `ZP_CLIENT_ID`/`ZP_CODIGO_INMOBILIARIA`.
2. `git pull` + reiniciar el backend en el VPS (queda pendiente el commit del polling de leads, ver abajo).
3. Apretar **"Importar leads históricos"** una sola vez (trae los 161 contactos de los últimos 90 días, sin mail) — después el cron de 15 minutos se encarga solo de los nuevos.
...
5. Correr "Sincronizar ZonaProp" / probar una propiedad puntual contra producción real recién cuando el usuario lo confirme explícitamente — a diferencia de sandbox, un error de mapeo acá sí puede afectar un aviso público real.

**Deploy 2026-09-02 (ya en producción):** commits `db598e0`, merge `2e4f241`, `bf810d8` (botón callbacks). **Pendiente de commitear/pushear** (2026-09-07): fix del timeout de `reconcile` (`e9398eb`, ya pusheado) + todo el trabajo de esta vuelta (polling de leads, límite de plan Free, botón "Importar leads históricos").
